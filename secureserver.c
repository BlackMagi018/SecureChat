...
int main(void) {
    ...
    serveraddr.sin_port = htons(9986);
            ...
            if (FD_ISSET (i, &tmp_set)) {
                if (i == sockfd) {
                    ...
                    if (keys[i][0] != '0') {
                        ...
                        memcpy(ciphertext, data + 20, ciphertext_len);
                        memset(data, 0, 5000);
                    ...
                    switch (control) {
                        ...
                        case 1:
                            ...
                            if (FD_ISSET(recipient, &sockets)) {
                                if (keys[recipient][0] != '0') {
                                    char ciphertext[1024];
                                    char temp[4];
                                    int ciphertext_len;
                                    RAND_pseudo_bytes(iv, 16);
                                    ciphertext_len = encrypt(data + 2, strlen(data + 2), keys[recipient], iv,
                                                             ciphertext);
                                    memset(data, 0, 5000);
                                    memcpy(data, iv, 16);
                                    ...
                                    char temp[1024];
                                    memcpy(temp, data + 2, 1024);
                                    memset(data, 0, 5000);
                                    memcpy(data, temp, 1024);
                                }
                                send(recipient, data, sizeof(data), 0);
                                memset(data, 0, 5000);
                            }
                            break;
                        case 2:
                                ...
                                if (FD_ISSET(loop, &sockets)) {
                                    if (loop != sockfd) {
                                        if (keys[loop][0] != '0') {
                                            ...
                                            int ciphertext_len;
                                            RAND_pseudo_bytes(iv, 16);
                                            ciphertext_len = encrypt(data + 1, strlen(data + 1), keys[loop], iv,
                                                                     ciphertext);
                                            memset(data, 0, 5000);
                                            memcpy(data, iv, 16);
                                            sprintf(temp, "%d", ciphertext_len);
                                            ...
                                            char temp[1024];
                                            memcpy(temp, data + 1, 1024);
                                            memset(data, 0, 5000);
                                            memcpy(data, temp, 1024);
                                        }
                                        send(loop, data, sizeof(data), 0);
                                    }
                                }
                            }
                            break;
                        case 3:
                            ...
                            if (keys[i][0] != '0') {
                                ...
                                int ciphertext_len;
                                RAND_pseudo_bytes(iv, 16);
                                ciphertext_len = encrypt(data, strlen(data), keys[i], iv, ciphertext);
                                memset(data, 0, 5000);
                                memcpy(data, iv, 16);
                                sprintf(temp, "%d", ciphertext_len);
                                for (int j = 0; j < 4; j++) {
                        ...
                        case 4:
                            ...
                            memset(data, 0, 5000);
                            sprintf(data, "Username set to %s", clients[i]);
                            if (keys[i][0] != '0') {
                                char ciphertext[1024];
                                char temp[4];
                                int ciphertext_len;
                        ...
                        case 5:
                            ...
                            memset(data, 0, 5000);
                            sprintf(data, "Confirm Kicking User %d with 6%d", user, user);
                            if (keys[i][0] != '0') {
                                char ciphertext[1024];
                                char temp[4];
                                int ciphertext_len;
